#include <vector>
#include <cmath>
#include <limits>
#include <algorithm>
#include <stdexcept>
using namespace std;

void bubbleSortT(vector<pair<int, int>>& arr);
void insertionSortT(vector<pair<int, int>>& arr);
void selectionSortT(vector<pair<int, int>>& arr);
void mergeSortT(vector<pair<int, int>>& arr);
void heapSortT(vector<pair<int, int>>& arr);
void quickSortT(vector<pair<int, int>>& arr);
void cocktailShakerSortT(vector<pair<int, int>>& arr);
void combSortT(vector<pair<int, int>>& arr);
void tournament_sortT(vector<pair<int, int>>& arr);
void introSortT(vector<pair<int, int>>& arr);
void librarySortT(vector<pair<int, int>>& arr);
void timSortT(vector<pair<int, int>>& arr);

// Bubble Sort
void bubbleSortT(vector<pair<int, int>>& arr) {
    int n = arr.size();
    for (int i = 0; i < n - 1; ++i) {
        for (int j = 0; j < n - i - 1; ++j) {
            // 첫 번째 원소만 비교하되, 안정성을 유지
            if (arr[j].first > arr[j + 1].first) {
                swap(arr[j], arr[j + 1]);
            }
        }
    }
}

// Insertion Sort
void insertionSortT(vector<pair<int, int>>& arr) {
    size_t n = arr.size();
    for (size_t i = 1; i < n; ++i) {
        pair<int, int> key = arr[i];
        int j = i - 1;
        while (j >= 0 && arr[j].first > key.first) { // 첫 번째 원소만 비교
            arr[j + 1] = arr[j];
            j--;
        }
        arr[j + 1] = key;
    }
}

// Selection Sort
void selectionSortT(vector<pair<int, int>>& arr) {
    size_t n = arr.size();
    for (size_t i = 0; i < n - 1; ++i) {
        size_t minIdx = i;
        for (size_t j = i + 1; j < n; ++j) {
            if (arr[j].first < arr[minIdx].first) // 첫 번째 원소만 비교
                minIdx = j;
        }
        swap(arr[i], arr[minIdx]);
    }
}

// --- Merge Sort ---
void merge(vector<pair<int, int>>& arr, int left, int mid, int right) {
    int n1 = mid - left + 1; 
    int n2 = right - mid;  
    vector<pair<int, int>> L(n1), R(n2);
    for (int i = 0; i < n1; ++i)
        L[i] = arr[left + i];
    for (int j = 0; j < n2; ++j)
        R[j] = arr[mid + 1 + j];
    int i = 0, j = 0, k = left;
    while (i < n1 && j < n2) {
        if (L[i].first <= R[j].first) // 첫 번째 원소만 비교
            arr[k++] = L[i++];
        else
            arr[k++] = R[j++];
    }
    while (i < n1)
        arr[k++] = L[i++];
    while (j < n2)
        arr[k++] = R[j++];
}

void merge_SortT(vector<pair<int, int>>& arr, int left, int right) {
    if (left < right) {
        int mid = left + (right - left) / 2;
        merge_SortT(arr, left, mid);       // 왼쪽 부분 정렬
        merge_SortT(arr, mid + 1, right);    // 오른쪽 부분 정렬
        merge(arr, left, mid, right);      // 두 정렬된 부분 병합
    }
}

void mergeSortT(vector<pair<int, int>>& arr) {
    if (arr.empty()) return;
    merge_SortT(arr, 0, arr.size() - 1);
}

// --- Heap Sort ---
void maxHeapify(vector<pair<int, int>>& arr, int n, int i) {
    int largest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;
    if (left < n && arr[left].first > arr[largest].first) // 첫 번째 원소만 비교
        largest = left;
    if (right < n && arr[right].first > arr[largest].first) // 첫 번째 원소만 비교
        largest = right;
    if (largest != i) {
        swap(arr[i], arr[largest]);
        maxHeapify(arr, n, largest);
    }
}

void buildMaxHeap(vector<pair<int, int>>& arr) {
    int n = arr.size();
    for (int i = n / 2 - 1; i >= 0; i--)
        maxHeapify(arr, n, i);
}

void heapSortT(vector<pair<int, int>>& arr) {
    int n = arr.size();
    buildMaxHeap(arr);
    for (int i = n - 1; i > 0; i--) {
        swap(arr[0], arr[i]);
        maxHeapify(arr, i, 0);
    }
}

// --- Quick Sort ---
int partition(vector<pair<int, int>>& arr, int low, int high) {
    pair<int, int> pivot = arr[high];
    int i = low;
    for (int j = low; j < high; j++) {
        if (arr[j].first < pivot.first) { // 첫 번째 원소만 비교
            swap(arr[i], arr[j]);
            i++;
        }
    }
    swap(arr[i], arr[high]);
    return i;
}

void _quick_sortT(vector<pair<int, int>>& arr, int low, int high) {
    if (low < high) {
        int pi = partition(arr, low, high);
        _quick_sortT(arr, low, pi - 1);
        _quick_sortT(arr, pi + 1, high);
    }
}

void quickSortT(vector<pair<int, int>>& arr) {
    if (arr.empty()) return;
    _quick_sortT(arr, 0, arr.size() - 1);
}

// --- Cocktail Shaker Sort ---
void cocktailShakerSortT(vector<pair<int, int>>& arr) {
    int n = arr.size();
    bool swapped = true;
    int start = 0, end = n - 1;
    while (swapped) {
        swapped = false;
        for (int i = start; i < end; ++i) {
            if (arr[i].first > arr[i + 1].first) { // 첫 번째 원소만 비교
                swap(arr[i], arr[i + 1]);
                swapped = true;
            }
        }
        if (!swapped) break;
        swapped = false;
        --end;
        for (int i = end; i > start; --i) {
            if (arr[i].first < arr[i - 1].first) { // 첫 번째 원소만 비교
                swap(arr[i], arr[i - 1]);
                swapped = true;
            }
        }
        ++start;
    }
}

// --- Comb Sort ---
void combSortT(vector<pair<int, int>>& arr) {
    int n = arr.size();
    int gap = n;
    const double shrink = 1.3;
    bool sorted = false;
    while (!sorted) {
        gap = int(gap / shrink);
        if (gap <= 1) {
            gap = 1;
            sorted = true;
        }
        for (int i = 0; i + gap < n; ++i) {
            if (arr[i].first > arr[i + gap].first) { // 첫 번째 원소만 비교
                swap(arr[i], arr[i + gap]);
                sorted = false;
            }
        }
    }
}

// --- Tournament Sort ---
void tournament_sortT(vector<pair<int, int>>& arr) {
    int n = arr.size();
    if (n == 0) return;
    int m = 1;
    while (m < n)
        m *= 2;
    const pair<int, int> INF = {numeric_limits<int>::max(), numeric_limits<int>::max()};
    vector<pair<int, int>> tree(2 * m, INF);
    for (int i = 0; i < m; i++) {
        if (i < n)
            tree[m + i] = arr[i];
        else
            tree[m + i] = INF;
    }
    for (int i = m - 1; i >= 1; i--) {
        tree[i] = (tree[2 * i].first < tree[2 * i + 1].first) ? tree[2 * i] : tree[2 * i + 1]; // 첫 번째 원소만 비교
    }
    for (int j = 0; j < n; j++) {
        pair<int, int> winner = tree[1];
        arr[j] = winner;
        int idx = 1;
        while (idx < m) {
            // 이 부분 때문에 항상 stable이 된다. 만약 오른쪽 부터 확인한다면 stable하지 않음.
            if (tree[2 * idx].first == winner.first)
                idx = 2 * idx;
            else
                idx = 2 * idx;
        }
        tree[idx] = INF;
        while (idx > 1) {
            idx /= 2;
            tree[idx] = (tree[2 * idx].first < tree[2 * idx + 1].first) ? tree[2 * idx] : tree[2 * idx + 1]; // 첫 번째 원소만 비교
        }
    }
}

// --- Intro Sort ---
void _insertionSortT(vector<pair<int, int>>& arr, int low, int high) {
    for (int i = low + 1; i <= high; i++) {
        pair<int, int> key = arr[i];
        int j = i - 1;
        while (j >= low && arr[j].first > key.first) { // 첫 번째 원소만 비교
            arr[j + 1] = arr[j];
            j--;
        }
        arr[j + 1] = key;
    }
}

void _maxHeapify(vector<pair<int, int>>& arr, int low, int heapSize, int i) {
    int largest = i;
    int relativeIndex = i - low;
    int left = low + 2 * relativeIndex + 1;
    int right = low + 2 * relativeIndex + 2;
    if (left < low + heapSize && arr[left].first > arr[largest].first) // 첫 번째 원소만 비교
        largest = left;
    if (right < low + heapSize && arr[right].first > arr[largest].first) // 첫 번째 원소만 비교
        largest = right;
    if (largest != i) {
        swap(arr[i], arr[largest]);
        _maxHeapify(arr, low, heapSize, largest);
    }
}

void _heapSortT(vector<pair<int, int>>& arr, int low, int high) {
    int heapSize = high - low + 1;
    for (int i = low + heapSize / 2 - 1; i >= low; i--)
        _maxHeapify(arr, low, heapSize, i);
    for (int i = high; i > low; i--) {
        swap(arr[low], arr[i]);
        heapSize--;
        _maxHeapify(arr, low, heapSize, low);
    }
}

void introsortUtil(vector<pair<int, int>>& arr, int low, int high, int max_depth) {
    if (high - low <= 16) {
        _insertionSortT(arr, low, high);
        return;
    }
    if (max_depth == 0) {
        _heapSortT(arr, low, high);
        return;
    }
    int pivot = partition(arr, low, high);
    introsortUtil(arr, low, pivot - 1, max_depth - 1);
    introsortUtil(arr, pivot + 1, high, max_depth - 1);
}

void introSortT(vector<pair<int, int>>& arr) {
    int n = arr.size();
    int max_depth = (n > 0) ? 2 * int(floor(log2(n))) : 0;
    introsortUtil(arr, 0, n - 1, max_depth);
}


// Library Sort
void librarySortT(vector<pair<int, int>>& arr) {
    int n = arr.size();
    int m = 2 * n; 
    vector<pair<int, int>> output(m, {INF, INF});  
    int count = 0;


    auto find_pos = [&](int x, int idx) {
        int left = 0, right = m - 1, res = m;
        while (left <= right) {
            int mid = (left + right) / 2;

            if (output[mid].first == INF || output[mid] > make_pair(x, idx)) {
                res = mid;
                right = mid - 1;
            } else {
                left = mid + 1;
            }
        }
        return res;
    };


    auto rebalance = [&]() {
        vector<pair<int, int>> temp(m, {INF, INF});
        int gap = 2;
        int idx = gap;
        for (auto& v : output) {
            if (v.first != INF) {
                if (idx >= m) throw runtime_error("Overflow during rebalance");
                temp[idx] = v;
                idx += gap;
            }
        }
        output = temp;
    };

    for (int i = 0; i < n; ++i) {
        int x = arr[i].first; 
        int idx = arr[i].second; 
        int pos = find_pos(x, idx); 


        int insert_pos = pos;
        while (insert_pos < m && output[insert_pos].first != INF)
            insert_pos++;

        if (insert_pos == m) {
            rebalance();
            insert_pos = find_pos(x, idx);
            while (insert_pos < m && output[insert_pos].first != INF)
                insert_pos++;
            if (insert_pos == m)
                throw runtime_error("Still no gap after rebalance");
        }

        for (int k = insert_pos; k > pos; --k)
            output[k] = output[k - 1];

        output[pos] = {x, idx}; 
        count++;
    }

    int idx = 0;
    for (auto& v : output)
        if (v.first != INF)
            arr[idx++] = v;
}


int computeMinRunT(int n) {
    int r = 0;
    while (n >= 64) {
        r |= (n & 1);
        n >>= 1;
    }
    return n + r;
}

void gallopMergeT(vector<pair<int,int>>& arr, int left, int mid, int right) {
    vector<pair<int,int>> L(arr.begin() + left, arr.begin() + mid + 1);
    vector<pair<int,int>> R(arr.begin() + mid + 1, arr.begin() + right + 1);
    int i = 0, j = 0, k = left;
    const int minGallop = 7; 
    while (i < L.size() && j < R.size()) {
        int count1 = 0, count2 = 0;

        while (i < L.size() && j < R.size()) {
            if (L[i].first <= R[j].first) {
                arr[k++] = L[i++];
                count1++;
                count2 = 0;
                if (count1 >= minGallop) break;
            } else {
                arr[k++] = R[j++];
                count2++;
                count1 = 0;
                if (count2 >= minGallop) break;
            }
        }
 
        if (i < L.size() && j < R.size() && count1 >= minGallop) {
            auto pos = upper_bound(L.begin() + i, L.end(), R[j]);
            int num = pos - (L.begin() + i);
            copy(L.begin() + i, pos, arr.begin() + k);
            k += num;
            i += num;
            continue;
        }

        if (i < L.size() && j < R.size() && count2 >= minGallop) {
            auto pos = lower_bound(R.begin() + j, R.end(), L[i]);
            int num = pos - (R.begin() + j);
            copy(R.begin() + j, pos, arr.begin() + k);
            k += num;
            j += num;
            continue;
        }
    }

    while (i < L.size())
        arr[k++] = L[i++];
    while (j < R.size())
        arr[k++] = R[j++];
}

struct RunT {
    int start;
    int length;
};

// Tim Sort
void timSortT(vector<pair<int,int>>& arr) {
    int n = arr.size();
    if (n < 2) return;
    
    int minRun = computeMinRunT(n);
    vector<RunT> runs;
    
    int i = 0;
    while (i < n) {
        int runStart = i;
        i++;
        if (i == n) {
            runs.push_back({runStart, 1});
            break;
        }
    
        if (arr[i - 1] <= arr[i]) {
            while (i < n && arr[i - 1] <= arr[i])
                i++;
        } else {
            while (i < n && arr[i - 1] > arr[i])
                i++;
            reverse(arr.begin() + runStart, arr.begin() + i);
        }
        int runLen = i - runStart;
        if (runLen < minRun) {
            int end = min(n, runStart + minRun);
            _insertionSortT(arr, runStart, end - 1);
            runLen = end - runStart;
            i = end;
        }
        runs.push_back({runStart, runLen});
    }
    
    // run들을 반복적으로 병합
    while (runs.size() > 1) {
        vector<RunT> newRuns;
        for (size_t j = 0; j < runs.size(); j += 2) {
            if (j + 1 < runs.size()) {
                int start = runs[j].start;
                int mid = runs[j].start + runs[j].length - 1;
                int end = runs[j + 1].start + runs[j + 1].length - 1;
                gallopMergeT(arr, start, mid, end);
                newRuns.push_back({start, runs[j].length + runs[j + 1].length});
            } else {
                newRuns.push_back(runs[j]);
            }
        }
        runs = move(newRuns);
    }
}